...
import { getStatusHistory, getFollowUps, sendFollowUp } from "../services/complaintService";

const STATUS_OPTIONS = ["pending", "reviewing", "resolved", "dismissed"];

export function ComplaintCard({ complaint, onStatusChange, onNoteAdd, onPublicMessageUpdate }) {
  const [expanded, setExpanded] = useState(false);
  const [note, setNote] = useState(complaint.adminNotes || "");
  const [publicMsg, setPublicMsg] = useState(complaint.publicMessage || "");
  const [statusHistory, setStatusHistory] = useState(complaint.statusHistory || []);
        ...
        <div className="p-8 sm:p-12 border-t border-slate-100 space-y-12 animate-in slide-in-from-top-4 duration-500">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            <div className="lg:col-span-2 space-y-10">
                      ...
                      return (
                        ...
                        >
                          <div className="absolute inset-0 flex flex-col items-center justify-center p-4">
                            ...
                            <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.1em]">Intelligence_{index + 1}</span>
                          </div>
                          ...
                          <img
                            ...
                            onLoad={(e) => {
                              ...
                              e.currentTarget.classList.remove('group-hover:opacity-10');
                            }}
                          />
                          ...
                          <div className="absolute inset-0 bg-slate-900/0 group-hover:bg-slate-900/60 transition-colors flex flex-col items-center justify-center">
                            ...
                            <span className="text-[8px] font-black text-white opacity-0 group-hover:opacity-100 uppercase tracking-widest mt-2">Open Intelligence</span>
                          </div>
                        </a>
                      );
                    })}
                ...
                {statusHistory && statusHistory.length > 0 ? (
                        ...
                        return (
                          <div key={idx} className="relative mb-5 last:mb-0">
                            ...
                            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1">
                              <p className={`text-[10px] font-black uppercase tracking-widest ${isLatest ? "text-slate-900" : "text-slate-400"}`}>
                                {entry.status}
                              </p>
                  ...
                  <div className="bg-slate-50 border border-slate-100 rounded-[2rem] p-6">
                    <p className="text-xs text-slate-400 italic">
                      No history recorded. Current status: <span className="font-black uppercase">{complaint.status}</span>
                    </p>
                  </div>
                )}
              </div>
              ...
              >
                <div className="flex items-center gap-3">
                  <div className={`w-8 h-8 rounded-xl flex items-center justify-center transition-colors duration-300 ${activeSection === 'followup' ? 'bg-red-600' : 'bg-slate-100'}`}>
                    <svg className={`w-4 h-4 transition-colors duration-300 ${activeSection === 'followup' ? 'text-white' : 'text-slate-900'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
                    </svg>
                  </div>
                  <p className={`text-xs font-black uppercase tracking-[0.2em] transition-colors duration-300 ${activeSection === 'followup' ? 'text-slate-900' : 'text-slate-400'}`}>
                    Internal Comms Protocol ({followUps.length})
                  </p>
                </div>

                <div className={`bg-slate-50 border transition-all duration-300 rounded-[2rem] p-6 space-y-4 max-h-80 overflow-y-auto ${activeSection === 'followup' ? 'border-red-200 shadow-lg shadow-red-900/5' : 'border-slate-100'}`}>
                  {followUps.length === 0 ? (
                    ...
                    followUps.map((msg) => (
                      ...
                      >
                        ...
                        >
                          <p className={`text-[9px] font-black uppercase tracking-widest mb-1 ${msg.sender === "admin" ? "text-slate-400" : "text-red-600"}`}>
                            {msg.sender === "admin" ? "Operator-Admin" : "Anonymous Source"}
                          </p>
                          <p className="text-sm font-medium leading-relaxed whitespace-pre-line">
                            {msg.message}
                          </p>
                          <p className={`text-[9px] mt-2 tabular-nums opacity-60 ${msg.sender === "admin" ? "text-slate-400" : "text-slate-400"}`}>
            ...
            <div className="space-y-10">
              ...
              >
                <label className={`block text-[10px] font-black uppercase tracking-[0.3em] pl-2 transition-colors duration-300 ${activeSection === 'status' ? 'text-slate-900 border-l-2 border-red-600' : 'text-slate-400'}`}>
                  Case Protocol State
                </label>
              ...
              >
                <label className={`block text-[10px] font-black uppercase tracking-[0.3em] pl-2 flex items-center gap-2 transition-colors duration-300 ${activeSection === 'public' ? 'text-red-600' : 'text-slate-900'}`}>
                ...
                >
                  <div className={`absolute inset-0 bg-red-600 transition-all duration-[400ms] ${isSavingPublic ? 'w-full opacity-50' : 'w-0 group-hover:w-full'}`} />
                  <span className="relative z-10 font-black">
                    {isSavingPublic ? "Transmitting..." : "Update Intelligence Feed"}
                  </span>
                </button>
              </div>
              ...
              >
                <label className={`block text-[10px] font-black uppercase tracking-[0.3em] pl-2 flex items-center gap-2 transition-colors duration-300 ${activeSection === 'note' ? 'text-slate-900' : 'text-slate-400'}`}>
                  <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.878 9.878L6.59 6.59m7.532 7.532l3.29 3.29M3 3l18 18" />
                  </svg>
                  Internal Notes
                </label>
                ...
                <button
                  ...
                  className="group w-full relative overflow-hidden bg-slate-900 text-white text-[10px] font-black uppercase tracking-[0.25em] py-4 px-6 rounded-2xl shadow-xl shadow-slate-900/10 hover:shadow-slate-900/20 active:translate-y-1 transition-all duration-300 cursor-pointer disabled:opacity-50"
                >
                  <div className={`absolute inset-0 bg-red-600 transition-all duration-[400ms] ${isSavingNote ? 'w-full opacity-50' : 'w-0 group-hover:w-full'}`} />
                  <span className="relative z-10">
                    {isSavingNote ? "Executing Sync..." : "Update Internal Notes"}
                  </span>
                </button>
              </div>
            </div>
